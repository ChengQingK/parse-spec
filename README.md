...
- “复杂词”专门展示较难理解的单个通用英文单词，与领域“术语”分开展示。顶栏“复杂词”可搜索、添加、编辑和删除，用户词表保存在根目录 `complex_words.json`；也可在句子分析顶部原文中右击单词快速添加，此时会优先用复杂词表、术语表和本地翻译词典自动填写中文释义，并在弹层中展示在线词典详情（音标、词性、英文释义、例句、同义词、常用搭配，数据来自 dictionaryapi.dev，缓存于 `word_cache.json`，离线时自动降级为仅本地释义）。内置词位于 `parse/complex_words.py`，难度标签只作为本工具的阅读提示，不冒充官方 CEFR 评级。
