...
Parse-Spec 是一个英文 SPEC PDF 长难句解析工具。浏览器使用 PDF.js 渲染 PDF、透明文本层和安全链接层；悬停可预览精确句子范围，单击后由 Flask 后端调用本地 spaCy 模型构建逻辑分句树，并在可停靠分析栏中展示中文辅助译文、语法证据和技术词释义。解析、术语与整句翻译默认本地执行（本地优先，保证响应速度）；联网能力仅作为增强：右击单词的词典详情（音标/例句/同义词等）与可选的在线精修会查询固定在线服务，带磁盘缓存、熔断与离线降级，主解析路径零等待。
...
- 本地结构翻译器结合逻辑分句、技术规则和用户/内置术语表生成完整及逐分句中文辅助译文（本地生成、零网络等待）；缩写（i.e./e.g.）、信号名与"数字+字母"复合词（2N、011b）保留原文或按词条翻译，中英文/信号名边界会保留可读空格，译文中仍保留英文的已知难词或术语可点击并原位替换成中文释义。
- 分析栏提供"标注异常"入口：读到翻译或解析不对的句子，可一键标注并附注意见，保存到 `data/sentence_feedback.json`（含原句与解析快照），便于后续批量分析改进解析质量。
- 术语抽取与 spaCy tokenizer/lemma 对齐；顶栏“术语表”可搜索内置/自定义词条，并将新增或覆盖内容保存到 `data/glossary.json`，立即用于当前句翻译。支持删除自定义词条；每次修改或删除前自动备份，也可手动备份、恢复、导出或删除备份，备份位于 `backups/glossary/`（最多 30 份，不提交 Git）。
- “复杂词”专门展示较难理解的单个通用英文单词，与领域“术语”分开展示。顶栏“复杂词”可搜索、添加、编辑和删除，用户词表保存在 `data/complex_words.json`；也可在句子分析顶部原文中右击单词快速添加，此时会优先用复杂词表、术语表和本地翻译词典自动填写中文释义，并在弹层中展示在线词典详情（音标、词性、英文释义、双语例句、同义词、常用搭配、在线中文释义，数据来自有道词典，dictionaryapi.dev 回退；缓存于 `data/word_cache.json`，离线时自动降级为仅本地释义）。内置词位于 `parse/complex_words.py`，难度标签只作为本工具的阅读提示，不冒充官方 CEFR 评级。
...
- 目录/书签共用左侧可拖动调宽的持久导航栏，跳转后不自动关闭；书签可自定义名称、记录页或精确句子，并统一保存到 `data/bookmarks.json`；旧版浏览器书签会按文档自动迁移。PDF 页面自带的内部目录、页码和安全外链也可点击。
...
- `parse/translator.py`：本地执行的术语保护、技术短语和逐分句辅助翻译。
